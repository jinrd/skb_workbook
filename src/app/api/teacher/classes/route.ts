...
export async function GET() {
  try {
    ...
    const whereCondition = session.role === 'ADMIN' ? {} : { teacherId: session.teacherId };
    ...
    return NextResponse.json({ error: '반 목록을 불러오는 중 오류가 발생했습니다.' }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const assignedTeacherId = (session.role === 'ADMIN' && teacherId) ? teacherId : session.teacherId;
